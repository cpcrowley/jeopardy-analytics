...
seasons from 1-31 + trebekpilots + superjeopardy
...
_.each(_.range(numberOfSeasons), function (seasonNumber) {

    // Special case to catch the differently named super jeopardy games
    if (seasonNumber === 0) { seasonNumber = 'superjeopardy'; }
    ...
    request(seasonUrl, function (seasonError, seasonResponse, seasonHtml) {
        //console.log('GOT SEASON: '+seasonUrl);
        if (seasonError) {
        ...
        seasonsProcessed += 1;
        //console.log('seasonsProcessed='+seasonsProcessed+' numberOfSeasons='+numberOfSeasons);
        ...
        console.log('Processed season ' + seasonNumber + ': ' + gameRecords.length +
                    ' games, fetching ' + fetchedGames + ' games');
    });
});
